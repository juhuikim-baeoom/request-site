...
- **병합 직전 최종 리뷰 지적 4건** (역할 모델 정교화 브랜치):
  ...
  - **[Important] 첨부 게이트 fail-open — 향후 댓글 삭제 기능 추가 시 내부메모 첨부 노출 위험** (`server/src/routes/attachments.ts`, `server/src/routes/request-detail.ts`): 다운로드 라우트의 `if (comment && !canSeeComment(...))`는 댓글 행을 못 찾으면 통과시켰고, 목록 라우트의 LEFT JOIN 결과 `?? false`는 조인 실패를 "공개"로 간주했다. 지금은 댓글 삭제 엔드포인트가 없어 악용 불가하지만, `comment_id`의 FK가 `onDelete: 'set null'`이라 나중에 댓글 삭제가 추가되는 순간 삭제된 내부메모의 첨부가 조용히 공개될 수 있는 구조였다. 두 지점 모두 **fail-closed**로 수정: `comment_id`가 non-null인데 댓글 행을 찾지 못하면 다운로드 거부(404)/목록 제외. `comment_id`가 null인 요청 본문 첨부는 기존처럼 정상 노출(과잉 차단 없음). 회귀 테스트 `server/scripts/test-attach-authz.ts`: `SET LOCAL session_replication_role = replica`로 FK 검증을 우회해 존재하지 않는 `comment_id`를 가리키는 첨부를 만들어 system을 포함한 전 역할에서 목록 제외·다운로드 거부(fail-closed)를 확인하고, `comment_id=null` 일반 첨부는 그대로 보임을 함께 확인.

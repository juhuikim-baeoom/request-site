...
### Fixed
- **병합 직전 최종 리뷰 지적 5건 — 공유 설정 사후 수정 브랜치** (`server/src/services/sharing.ts`, `server/scripts/test-sharing.ts`, `server/scripts/test-api-write.ts`, `src/features/requests/RequestForm.tsx`, `CHANGELOG.md`):

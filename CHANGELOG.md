...
### Fixed
- **요청 접수 폼(태블릿·모바일) — 하단 고정 제출바가 `속성·공유` 카드를 가리고 첨부↔카드 사이에 빈 공간이 생기던 문제** (`src/features/requests/RequestForm.tsx`): 고정 제출바(높이 ~65px)를 위한 여백 스페이서가 폼 맨 아래가 아니라 작성 컬럼 끝(첨부 다음)에 있었다. 단일 컬럼(<lg)에서는 작성 컬럼과 속성 카드가 위아래로 쌓이므로, 이 스페이서가 첨부와 속성 카드 사이 **중간에 끼어** 빈 공간을 만들고, 정작 폼의 실제 하단인 속성 카드에는 여백이 없어 **공유대상 검색칸이 고정 바에 가려졌다**(스페이서 56px도 바 높이 65px보다 짧았다). 스페이서를 폼 맨 아래(속성 카드 다음)로 옮기고 72px로 키웠다. lg 이상은 카드 안 인라인 제출 버튼을 쓰므로 무영향. 폭 390·900·1440px에서 겹침 없음·데스크톱 무변경 확인.

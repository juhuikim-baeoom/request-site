...
**후보 목록에 없는 저장값도 칩으로 표시하고 해제할 수 있다.** 칩은 후보 목록(`allCandidates`)이 아니라 부모 Set(`fnTargets`·`deptTargets`)을 기준으로 만든다 — 저장된 값이 조직도 변경으로 후보에서 빠졌거나 `useDeptOptions()`가 아직 로딩/실패 중이어도 칩이 사라지면 안 된다(칩이 안 보이는데 부모 Set에는 값이 남아 있으면, 사용자는 공유가 없다고 믿지만 저장 시 그 공유가 그대로 되살아나는 유령 공유가 된다). 후보 목록에서 라벨을 찾되 없으면 값 자체로 라벨을 만든다(세부부서는 `parseDeptTargetValue()`로 "기관 › 팀" 형식, 직무는 "값 전체"). `target_value`는 이 경우에도 절대 새로 만들지 않는다 — 라벨만 대체한다.

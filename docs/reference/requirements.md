...
- **역할별 메뉴 노출**
  - 전 역할(staff·dept_monitor·org_monitor·system·exec·system_admin): 접수 폼, 내 요청 목록
  - `canProcess`(system·system_admin): 상기 + 관리 보드
...
- **탭**: 내 요청 / (역할에 따라 라벨이 달라지는) 두 번째 탭 — `canSeeAllRequests`(system·system_admin·exec)는 "전체 요청", `org_monitor`는 "우리 기관 요청", `dept_monitor`는 "우리 부서 요청", 그 외(staff)는 "부서·공유 요청". 실제 열람 범위는 서버 `visibilityFilter`가 결정한다(§1 참조).
...
- **되돌아가기 목적지 분기**: 목록에서 상세로 들어올 때 진입 경로를 쿼리 파라미터로 넘긴다(관리 보드 `?from=board`, 내 요청 목록 `?from=mine`). 상세 상단 링크는 이 값에 따라 "← 관리 보드"(`/board`) 또는 "← 내 요청 목록"(`/requests/mine`)으로 렌더링한다. 값이 없거나(알림 벨 진입 등) 알 수 없는 값, 또는 `from=board`인데 `canProcess`가 없으면 내 요청 목록으로 폴백한다. 관리 보드 필터는 기존 localStorage 복원 로직으로 유지된다.

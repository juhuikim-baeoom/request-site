...
- **관리 패널(시스템팀 전용, `src/features/requests/AdminPanel.tsx`)**: 요약 영역 아래 담당자·상태·영향도 select. 담당자는 `PATCH /api/requests/:id { assignee_id }`. 상태는 `ALLOWED_TRANSITIONS` 기준으로 불허 전이를 select에서 비활성화("(불가)" 표기)하며, 보류·반려로의 전이는 사유 입력 모달을 거친다. 영향도는 `PATCH /api/requests/:id/impact`(시스템팀 전용, body `{ impact: 높음|보통|낮음 }`)로 조정하며 미배정 건은 select 비활성+400 `NOT_ASSIGNED`, 종결(완료·반려·철회) 건은 400 `CLOSED`로 거부한다. 성공 시 `priority_level`·`response_due_at`·`resolution_due_at`·`sla_policy_id`·`sla_response_breached`를 재산정하고 `assigned_at`·`first_response_at`·`status`는 보존한다(계산은 배정 API와 `server/src/services/sla-fields.ts`를 공유).
- **재작업 버튼**: 시스템팀 & `status='완료'` → PATCH `{ status:'진행중', reason? }`. 사유 입력 선택.
- **요청자 액션**: `status='접수'`일 때 수정(title/body/urgency/visibility/desired_due) / 철회(PATCH `{ status:'철회' }`). 편집과 상태변경 분리(서버 400 회피). **필드 편집 권한**: 시스템팀 또는 (요청자 본인 && `status='접수'`)일 때 가능 — 시스템팀은 상태 무관하게 편집 가능하도록 확장됨. 철회 버튼은 `ALLOWED_TRANSITIONS`상 '접수' 상태에서만 노출.
...
- **트리아지 존(미배정 큐)**: `status='접수' && assignee_id 없음` 건을 보드 상단에 별도 표시. "배정" 버튼 클릭 시 담당자+영향도 선택 모달 → `POST /api/requests/:id/assign` 호출. 배정 완료 시 진행중으로 자동 전이. 미배정 큐도 드롭 대상이며, 진행중 카드를 큐에 놓으면 배정 취소(→ 접수)로 처리된다. 드래그 오버 시 "여기에 놓기" 텍스트를 표시한다.

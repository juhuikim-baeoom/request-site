...
첨부파일 게이트는 **fail-closed**다: 첨부의 `comment_id`가 non-null인데(내부메모에 딸린
첨부) 그 댓글 행을 찾지 못하면(현재는 댓글 삭제 API가 없어 발생하지 않지만, 나중에
생기면 `comment_id`의 FK `onDelete: 'set null'`로 인해 이 경로를 탈 수 있다) 목록에서
제외·다운로드 거부한다 — "찾을 수 없음"을 "공개"로 fail-open 처리하지 않는다. `comment_id`가
애초에 null인 요청 본문 첨부는 그대로 노출된다(과잉 차단 아님).

-- Custom SQL migration file: triggers, functions, view --
...
-- 상태 변경 처리: 이력 기록(변경자=app.user_id) + 완료일/재작업/SLA 관리
...
select
  ...
  case
    ...
    when r.resolution_due_at is not null and now() > r.resolution_due_at then '초과'
